import Link from "next/link";

const SocialSection = () => {
  return(
  <div className="pt-2 font-medium text-muted-foreground px-2">
    <p>
      {" "}
      You can find me on{" "}
      <span className="font-semibold border-b">
        <Link href="https://x.com/santosh_marar">X</Link>
      </span>
      ,{" "}
      <span className="font-semibold border-b">
        <Link href="https://github.com/santosh-marar">GitHub</Link>
      </span>
      ,{" "}
      <span className="font-semibold border-b">
        <Link href="https://www.linkedin.com/in/santosh-kumar-marar-98536319b/">
          LinkedIn
        </Link>
      </span>
      , or schedule a meetup on{" "}
      <span className="text-primary font-semibold border-b border-dotted">
        <Link href="https://cal.com/santosh-marar/15-min-meeting">Cal.com</Link>
      </span>
    </p>
  </div>
)
};

export default SocialSection;
